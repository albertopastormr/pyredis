...
class TestBlpopTimeout:
    ...
    def test_blpop_multiple_timeouts_different_durations(self):
        ...
        async def test():
            ...
            blpop1 = asyncio.create_task(
                async_execute_command(['BLPOP', 'list', '0.2'])
            )
            blpop2 = asyncio.create_task(
                async_execute_command(['BLPOP', 'list', '0.5'])
            )
            blpop3 = asyncio.create_task(
                async_execute_command(['BLPOP', 'list', '1.0'])
            )
    ...
    def test_blpop_timeout_precision(self):
        ...
        async def test():
            ...
            for timeout_val in timeouts:
                start = time.time()
                result = await async_execute_command(['BLPOP', f'key_{timeout_val}', str(timeout_val)])
                elapsed = time.time() - start
...
class TestBlpopTimeoutEdgeCases:
    ...
    def test_blpop_respects_fifo_order_with_timeout(self):
        ...
        async def test():
            # Start 3 waiters
            waiters = [
                asyncio.create_task(async_execute_command(['BLPOP', 'fifo', '2']))
                for _ in range(3)
            ]
