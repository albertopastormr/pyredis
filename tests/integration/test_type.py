...
class TestTypeIntegration:
    ...
    def test_type_multiple_keys_different_types(self):
        ...
        assert execute_command(["TYPE", "str_key"]) == {"ok": "string"}
        assert execute_command(["TYPE", "list_key"]) == {"ok": "list"}
        assert execute_command(["TYPE", "no_key"]) == {"ok": "none"}
    ...
    def test_type_after_set_overwrites_list(self):
        ...
        assert execute_command(["TYPE", "mykey"]) == {"ok": "list"}

        execute_command(["SET", "mykey", "string_value"])
        assert execute_command(["TYPE", "mykey"]) == {"ok": "string"}
    ...
    def test_type_case_insensitive(self):
        ...
        assert result1 == result2 == result3 == {"ok": "string"}

    def test_type_returns_simple_string_in_resp(self):
        """TYPE returns simple string in RESP format."""
        execute_command(["SET", "key", "value"])
        result = execute_command(["TYPE", "key"])

        # Verify it's the dict format for simple string
        assert result == {"ok": "string"}

        # Verify RESP encoding as simple string
        from app.resp import RESPEncoder

        response = RESPEncoder.encode(result)
        assert response == b"+string\r\n"  # Simple string, not bulk string
